...
function get_page_title() 
{
    ...
    $page_titles = [
        ...
        '404' => '404 Oldal nem található - Villám Meló'
    ];
    
    if (array_key_exists($relative_path, $page_titles)) {
        return $page_titles[$relative_path];
    } else {
        return '404 Oldal nem található - Villám Meló';
    }
}
...
function update_profile() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['profil_modositasa'])) {
        ...
        $beerkezett_varmegye = trim($_POST['varmegye'] ?? '');
        if ($beerkezett_varmegye === 'no_match') {
            $beerkezett_varmegye = '';
        }
        ...
        if (!empty($uj_adatok['varmegye']) && !in_array($uj_adatok['varmegye'], $varmegyek)) {
            $hibak[] = "Érvénytelen vármegye! Kérjük, válassz a listából egy érvényes vármegyét.";
        }
            ...
            try {
                ...
                if ($check_table->rowCount() == 0) {
                    $create_table_sql = "CREATE TABLE IF NOT EXISTS kapcsolat_uzenet (
                        kuid INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        fid INT(11) NOT NULL,
                ...
                $check_auto_increment = $conn->query("SELECT EXTRA FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'kapcsolat_uzenet' AND COLUMN_NAME = 'kuid'");
                $column_info = $check_auto_increment->fetch();
                
                if ($column_info && strpos($column_info['EXTRA'], 'auto_increment') === false) {
                    $alter_sql = "ALTER TABLE kapcsolat_uzenet MODIFY kuid INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY";
                    $conn->exec($alter_sql);
                }
                ...
                $sql = "INSERT INTO kapcsolat_uzenet (fid, nev, email, kategoria, uzenet, idopont) VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->execute([$fid, $name, $email, $category, $message, $idopont]);
...
function getFooterStats() {
    ...
    $stats = [
        'tagok_szama' => 0,
        'munkak_szama' => 0,
        'elegedett_ugyfelek_szama' => 0
    ];
    
    try {
        $sql_tagok = "SELECT COUNT(*) AS tagok_szama FROM felhasznalok WHERE statusz = 'Aktiv'";
        $stmt_tagok = $conn->query($sql_tagok);
        $tagok = $stmt_tagok->fetch(PDO::FETCH_ASSOC);
        $stats['tagok_szama'] = $tagok['tagok_szama'] ?? 0;
        
        $sql_munkak = "SELECT COUNT(*) AS munkak_szama FROM munkak WHERE aktiv = 1 AND statusz = 'aktiv'";
        $stmt_munkak = $conn->query($sql_munkak);
        $munkak = $stmt_munkak->fetch(PDO::FETCH_ASSOC);
        $stats['munkak_szama'] = $munkak['munkak_szama'] ?? 0;
        
        $sql_ugyfelek = "SELECT COUNT(DISTINCT fid) AS elegedett_ugyfelek FROM megjegyzesek WHERE ertekeles >= 4";
        $stmt_ugyfelek = $conn->query($sql_ugyfelek);
        $ugyfelek = $stmt_ugyfelek->fetch(PDO::FETCH_ASSOC);
        $stats['elegedett_ugyfelek_szama'] = $ugyfelek['elegedett_ugyfelek'] ?? 0;
        
    } catch (PDOException $e) {
        error_log("Footer statisztika hiba: " . $e->getMessage());
    }
