<?php
// jobs.php - Javított CSS: Alapértelmezett világos téma kényszerítése
...
if (!$db_connected) {
    ...
    try {
        ...
        // Hiba esetén később jelezzük
    }
}

// --- 2. KATEGÓRIA DEFINÍCIÓK ---
$kilteri_definicio = [
    ...
    try {
        ...
        if (count($munkak) > 0) {
            ...
            foreach ($munkak as $munka) {
                // Kép kezelés: Ha nincs kép, 'default_job.png' jön vagy üres
                $kepek = !empty($munka['kepek']) ? explode('|', $munka['kepek']) : ['default_job.png'];
                $kep_path = 'assets/images/referencia_kepek/';
                
                $output .= '<div class="job-listing-card">';
                
                // Slider
                $output .= '<div class="job-card-slider">';
                if (count($kepek) > 1) {
                    $output .= '<button class="slider-btn prev" type="button" onclick="moveSlide(this, -1)">&#10094;</button>';
                    $output .= '<button class="slider-btn next" type="button" onclick="moveSlide(this, 1)">&#10095;</button>';
                }
                $output .= '<div class="slider-track" data-current="0">';
                
                foreach ($kepek as $kep) {
                    // Itt ellenőrizzük, hogy van-e valódi kép
                    if ($kep == 'default_job.png' || empty($kep)) {
                        // Nincs kép -> Témához illő színes doboz felirattal
                        $output .= '<div class="slider-img no-image-placeholder">';
                        $output .= '<i class="fa fa-camera-retro"></i>'; // Ikon
                ...
                $output .= '<a href="work?job_id=' . $munka['id'] . '" class="btn-details">Részletek</a>';
                $output .= '</div>';
            ...
            return '<div class="no-jobs-found">Nincs megjeleníthető aktív munka a megadott névvel/szűréssel.</div>';
        }
    } catch (PDOException $e) {
...
// --- 5. DINAMIKUS LISTÁK BETÖLTÉSE ---
$elerheto_kategoriak_db = [];
if ($db_connected) {
    try {
        $stmt_cat = $conn->query("SELECT DISTINCT munka_nev FROM munkak WHERE statusz = 'Aktiv' ORDER BY munka_nev ASC");
...
<style>
    ...
    /* * TÉMA KEZELÉS JAVÍTVA
     * Alapértelmezésben MINDIG világos színeket használunk.
     * Csak akkor váltunk sötétre, ha a body-n rajta van a dark class.
     */
    :root {
        --placeholder-bg: #fff8e1; /* Világos sárga - Ez az alap */
        --placeholder-text: #b38b00;
        --placeholder-icon: #ffbe33;
    }

    /* Csak akkor sötét, ha explicit be van kapcsolva az oldalon */
    ...
    .job-listing-card:hover {
        ...
        box-shadow: 0 10px 25px var(--shadow-color, rgba(0,0,0,0.15));
        border-color: var(--accent-color, #667eea);
    }
    ...
    /* --- TÉMA-ÉRZÉKENY HELYŐRZŐ STÍLUS --- */
    .no-image-placeholder {
        width: 100%;
        min-width: 100%;
        height: 100%;
        
        /* Itt használjuk a változókat */
    ...
    .no-jobs-found {
        ...
        text-align: center;
        padding: 50px 20px;
        background: var(--tertiary-bg, #f5f5f5);
        border-radius: 10px;
        color: var(--secondary-text, #666);
    }
</style>
