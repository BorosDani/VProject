...
// Segédfüggvény érvénytelen dátumok ellenőrzésére
...
// Dátum formázása - TELJESEN JAVÍTOTT VERZIÓ
...
// Felhasználók lekérése rendezéssel - JAVÍTOTT VERZIÓ
function get_users($page = 1, $per_page = 15, $search = '', $status = '', $order_by = 'regisztralt', $order_dir = 'DESC') {
    ...
    // Adatok lekérése - JAVÍTVA: hozzáadva statusz_ok és statusz_meddig
...
// Felhasználó adatainak frissítése - JAVÍTOTT VERZIÓ
function update_user_data($user_id, $data) {
    global $conn;
    
    try {
        // Felhasználó aktuális adatainak lekérése
...
// Felhasználó státusz módosítása - JAVÍTVA: email_megerositve frissítése
function update_user_status($user_id, $status, $reason = '', $until = null) {
    global $conn;
    
    try {
        // Ha státusz "Függőben"-re változik, akkor email_megerositve = 0
        ...
        if ($success) {
            log_admin_activity(
                ...
                $reason . ($status === 'Fuggoben' ? ' (Email megerősítés visszaállítva)' : '')
            );
        }
...
function delete_activity($activity_id, $table) {
    ...
    // Tábla nevek és megfelelő ID oszlopok definiálása
    $table_config = [
...
// Felhasználó adatainak lekérése ID alapján - JAVÍTOTT VERZIÓ
function get_user_by_id($user_id) {
    global $conn;
    
    try {
        $sql = "SELECT fid, fnev, email, knev, vnev, nem, szuletett, telefon, varos, reszletek, 
                       szerep, statusz, profilkep, regisztralt, modositott, belepett,
                       email_megerositve, statusz_ok, statusz_meddig
                FROM felhasznalok 
                WHERE fid = ?";
        $stmt = $conn->prepare($sql);
...
// Lapozó generálása - JAVÍTOTT VERZIÓ
function generate_pagination($current_page, $total_pages, $base_url) {
    ...
    if ($current_page > 1) {
        ...
        $pagination .= '<li><a href="' . htmlspecialchars($prev_url) . '">&laquo;</a></li>';
    }
    
    // Első oldal
    if ($start_page > 1) {
        $first_url = $base_url . '&page=1';
        $pagination .= '<li><a href="' . htmlspecialchars($first_url) . '">1</a></li>';
        if ($start_page > 2) {
    ...
    for ($i = $start_page; $i <= $end_page; $i++) {
        ...
        $pagination .= '<li><a href="' . htmlspecialchars($page_url) . '" class="' . $active . '">' . $i . '</a></li>';
    }
    ...
    if ($end_page < $total_pages) {
        ...
        $pagination .= '<li><a href="' . htmlspecialchars($last_url) . '">' . $total_pages . '</a></li>';
    }
    
    // Következő oldal gomb
    if ($current_page < $total_pages) {
        $next_url = $base_url . '&page=' . ($current_page + 1);
        $pagination .= '<li><a href="' . htmlspecialchars($next_url) . '">&raquo;</a></li>';
    }
...
// Email ellenőrzés státusz formázása - JAVÍTOTT
function format_email_status($verification) {
    ...
    // Ellenőrizzük, hogy van-e lejárati idő és az érvényes-e
    if (empty($verification['lejarati_ido']) || 
    ...
    if ($lejarati_ido === false || $lejarati_ido < 0) {
        return '<span class="admin-badge admin-badge-warning">Függőben</span>';
    }
    
    if ($lejarati_ido < time()) {
...
// Jelszó visszaállítás státusz formázása - JAVÍTOTT
...
// URL paraméterek biztonságos összeállítása
function build_query_params($base_params, $additional_params = []) {
    $params = array_merge($base_params, $additional_params);
    
    // Eltávolítjuk az üres értékeket
