        ...
        if (empty($hibak)) {
            try {
                ...
                if ($regisztralt_de_nem_megerositett) {
                    ...
                    // Új regisztráció - email_megerositve = 0 alapértelmezetten
                    $jelszo_hash = password_hash($jelszo, PASSWORD_DEFAULT);
                    $sql = "INSERT INTO felhasznalok (email, fnev, jelszo, profilkep, statusz, email_megerositve) VALUES (?, ?, ?, 'default.png', 'aktiv', 0)";
...
function login() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['log_button'])) {
        ...
        if (empty($hibak)) {
            try {
                // MOST MÁR a felhasznalok táblából ellenőrizzük az email_megerositve mezőt
                ...
                if ($felhasznalo) {
                    // Email megerősítés ellenőrzése - MOST MÁR a felhasznalok táblából
                    if ($felhasznalo['email_megerositve'] == 0) {
                        ...
                        */
                        ...
                        $_SESSION['varos'] = $felhasznalo['varos'] ?? '';
...
function email_megerosites($token) {
    ...
    // Email megerősítése - MINDKÉT táblában frissítjük
    ...
    try {
        ...
        // 2. felhasznalok tábla frissítése - email_megerositve = 1
        $update_felhasznalo_sql = "UPDATE felhasznalok SET email_megerositve = 1 WHERE fid = ?";
...
function uj_aktivacios_email_kuldes($email, $fid = null) {
    ...
    if (!$fid) {
        ...
        $sql = "SELECT fnev, email FROM felhasznalok WHERE fid = ? AND statusz = 'aktiv'";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$fid]);
        $felhasznalo = $stmt->fetch();
        
        if (!$felhasznalo) {
            return ['sikeres' => false, 'uzenet' => 'Nem található felhasználó.'];
        }
...
// PROFIL MÓDOSÍTÁS FUNKCIÓ
// PROFIL ADATOK FRISSÍTÉSE - JAVÍTOTT VERZIÓ
function update_profile() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['profil_modositasa'])) {
        ...
        try {
            $sql = "SELECT nem, email, fnev, knev, vnev, profilkep, szuletett, telefon, varos FROM felhasznalok WHERE fid = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$fid]);
        ...
        if (empty($hibak)) {
            try {
                $sql = "UPDATE felhasznalok SET nem = ?, email = ?, fnev = ?, knev = ?, vnev = ?, szuletett = ?, telefon = ?, varos = ?, modositott = CURRENT_TIMESTAMP";
                $params = [
                    $uj_adatok['nem'], 
                    $uj_adatok['email'], 
                    $uj_adatok['fnev'], 
                ...
                if ($sikeres_frissites) {
                    ...
                    $_SESSION['varos'] = $uj_adatok['varos'];
                    ...
                    if ($uj_adatok['email'] !== $jelenlegi_adatok['email']) {
                        $token = general_aktivacios_token();
                        mentes_aktivacios_token($fid, $token);
                        kuldes_aktivacios_email($uj_adatok['email'], $uj_adatok['fnev'], $token);
                        
                        $siker .= " Mivel megváltoztattad az email címed, küldtünk egy megerősítő emailt az új címre!";
                    }

                    // AZONNALI FRISSÍTÉS - újra lekérjük a felhasználó adatait
                    $refresh_sql = "SELECT fid, szerep, statusz, nem, email, fnev, knev, vnev, profilkep, szuletett, telefon, varos, regisztralt, modositott, belepett, email_megerositve 
                                   FROM felhasznalok 
                                   WHERE fid = ?";
...
// MUNKÁK FUNKCIÓK - EZEKET ADD HOZZÁ A FUNCTIONS.PHP VÉGÉRE
